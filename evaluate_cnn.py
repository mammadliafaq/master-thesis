...
def evaluate(args: argparse.Namespace) -> None:
    ...
    image_embeddings = generate_image_features(config, model, valid_loader, device)
    ...
    if config.search_for_threshold:
        ...
        f1_score, output_df = get_image_predictions(
            valid, image_embeddings, threshold=config.threshold
        )
        print(output_df.head())
        submit_df = output_df[["posting_id", "pred_img_only", "target"]].copy()
        submit_df.rename(columns={"pred_img_only": "matches"}, inplace=True)
